...
import RegisterPopup from "../RegistePopup";
...
const ChatDropdown = ({ currentUser }: { currentUser: string }) => {
  ...
  return (
    <Menu>
      ...
      >
        <Flex>
          {data?.getConversations[0]?.isNew &&
          data?.getConversations[0]?.reciever == currentUser ? (
            <Circle
              size="10px"
              position="absolute"
              top={1}
            ...
            <Flex minW={"full"} align={"center"} flexDirection={"row"}>
              ...
              <Flex ml={2} flexDirection={"column"}>
                <Flex align={"center"} flexDirection={"row"}>
                  ...
                  {convo.isNew ? (
                    <Circle ml={2} size="10px" bg="red.500" />
                  ) : null}
                </Flex>
...
const ChatNavbar = () => {
  ...
  const { data: DATA, loading: LOADING } = useQuery(NAVBARQUERY);
